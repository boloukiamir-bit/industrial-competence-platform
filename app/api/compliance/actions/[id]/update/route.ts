 ...
 * Admin/HR only. Org-scoped. If activeSiteId set, action.site_id must match (prevent cross-site edits).
 */
...
export async function POST(
  ...
  if (due_date === undefined && notes === undefined && owner_user_id === undefined) {
    const res = NextResponse.json(errorPayload("validation", "At least one of due_date, notes, owner_user_id required"), {
      status: 400,
    });
    applySupabaseCookies(res, pendingCookies);
    return res;
