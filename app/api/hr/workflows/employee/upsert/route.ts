...
export const POST = withMutationGovernance(
  async (ctx) => {
    ...
    const body = ctx.body as { employee_id?: string; workflow_code?: string; step_code?: string; status?: string; due_date?: string | null; notes?: string | null; evidence_url?: string | null };
    ...
    try {
      ...
      [ctx.orgId, workflow_code, step_code]
    );
      ...
      await pool.query(
        `INSERT INTO employee_hr_step_status (org_id, employee_id, workflow_id, step_id, status, due_date, notes, evidence_url, updated_at)
         ...
         ON CONFLICT (org_id, employee_id, step_id) DO UPDATE SET
           ...
           updated_at = now()`,
        [ctx.orgId, employee_id, workflow_id, step_id, status, due_date, notes, evidence_url]
      );
  ...
  {
    route: "/api/hr/workflows/employee/upsert",
    action: "HR_WORKFLOW_EMPLOYEE_UPSERT",
    target_type: "org",
    allowNoShiftContext: true,
    getTargetIdAndMeta: (body) => ({
      target_id: typeof (body as { employee_id?: string }).employee_id === "string" ? `employee:${(body as { employee_id: string }).employee_id}` : "unknown",
      meta: {},
    }),
  }
);
