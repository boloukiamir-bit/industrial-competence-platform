...
import type { InboxActionItem, InboxLifecycleItem, InboxGovernanceItem, InboxContractItem, InboxMedicalItem, PrioritySummary } from "@/types/domain";
...
export default function HrInboxPage() {
  ...
  const [items, setItems] = useState<InboxActionItem[] | InboxLifecycleItem[] | InboxGovernanceItem[] | InboxContractItem[] | InboxMedicalItem[]>([]);
  ...
  const effectiveTab = useMemo((): InboxTab => {
    if (tab === "lifecycle" || tab === "governance" || tab === "contract" || tab === "medical") return tab;
    return "actions";
  }, [tab]);
