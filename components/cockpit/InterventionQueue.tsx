...
export function InterventionQueue({
  ...
  return (
    <div className="cockpit-card-secondary p-0 overflow-hidden mb-6" data-testid="intervention-queue">
      <div className="px-4 py-2 border-b border-border">
        <p className="cockpit-title">Intervention Queue</p>
      </div>
          ...
          const leftRule = !acceptedRisk && issue.severity === "BLOCKING"
            ...
            : !acceptedRisk && issue.severity === "WARNING"
              ...
              : acceptedRisk
                ...
                : "";
          return (
            <li key={issue.issue_id} className={cn("px-4 py-2", marked && "bg-muted/20", leftRule)}>
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="cockpit-body font-medium">{stationLabel}</span>
                    {issue.area && <span className="cockpit-label">{issue.area}</span>}
                    {acceptedRisk && (
                      <span className="cockpit-label cockpit-status-at-risk">Accepted</span>
                    )}
                  </div>
                  <p className="cockpit-body text-muted-foreground mt-0.5">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <Button
                    ...
                    className="h-7 text-[13px]"
                    onClick={() => onMarkPlanned(issue.issue_id)}
