...
-- 5. Person Events (People Risk / Compliance Engine)
...
-- 7. Employee Equipment assignments
...
CREATE TABLE IF NOT EXISTS documents (
  ...
  employee_id UUID REFERENCES employees(id) ON DELETE CASCADE,
  title TEXT NOT NULL,
  type TEXT CHECK (type IN ('contract', 'handbook', 'policy', 'certificate', 'other')),
  url TEXT NOT NULL,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  valid_to DATE
);
...
-- Seed some sample role skill requirements
INSERT INTO role_skill_requirements (role, line, skill_id, required_level, required_headcount)
SELECT 'Operator', 'Pressline 1', id, 3, 2 FROM skills WHERE code = 'PRESS_A'
ON CONFLICT DO NOTHING;

INSERT INTO role_skill_requirements (role, line, skill_id, required_level, required_headcount)
SELECT 'Operator', 'Assembly', id, 2, 3 FROM skills WHERE code = '5S'
ON CONFLICT DO NOTHING;
