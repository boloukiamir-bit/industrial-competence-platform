...
async function getAllPositions() {
    const { data, error } = await supabase.from('positions').select('id, name, site, department').order('name', {
        ascending: true
    });
...
async function getEmployeesForPosition(positionId) {
    const { data, error } = await supabase.from('employees').select('id, full_name, name').eq('position_id', positionId);
    if (error) throw error;
    return (data ?? []).map((row)=>({
            id: row.id,
...
async function getEmployeeCompetenceProfile(employeeId) {
    // 1) Employee
    const { data: employeeRow, error: empError } = await supabase.from("employees").select("id, full_name, name, position_id").eq("id", employeeId).single();
    ...
    if (positionId) {
        const { data: posRows } = await supabase.from("positions").select("id, name").eq("id", positionId).limit(1);
        const pos = posRows?.[0];
        if (pos) {
    ...
    let requirementRows = [];
    if (positionId) {
        const { data: reqData, error: reqError } = await supabase.from("position_competence_requirements").select("id, position_id, competence_id, required_level, mandatory").eq("position_id", positionId);
        if (reqError) {
    ...
    const { data: empCompData, error: empCompError } = await supabase.from("employee_competences").select("id, employee_id, competence_id, level, valid_from, valid_to").eq("employee_id", employeeId);
    if (empCompError) {
    ...
    if (competenceIds.size > 0) {
        const { data: compData, error: compError } = await supabase.from("competences").select("id, name, description, code, group_id, is_safety_critical").in("id", Array.from(competenceIds));
        if (compError) {
        ...
        if (groupIds.size > 0) {
            const { data: groupData, error: groupError } = await supabase.from("competence_groups").select("id, name").in("id", Array.from(groupIds));
            if (groupError) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__adce9ae0._.js.map