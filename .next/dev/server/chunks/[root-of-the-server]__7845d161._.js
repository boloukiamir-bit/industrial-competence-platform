...
"[project]/app/api/line-overview/demand/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function POST(request) {
    try {
        ...
        const existingCheck = await supabaseAdmin.from("pl_machine_demand").select("id").eq("org_id", DEMO_ORG_ID).eq("machine_code", machineCode).eq("plan_date", date).eq("shift_type", shiftType).maybeSingle();
        if (existingCheck.data) {
            const { data, error } = await supabaseAdmin.from("pl_machine_demand").update({
                required_hours: requiredHours,
                priority: priority || 1,
                comment: comment || null
            }).eq("id", existingCheck.data.id).select().single();
        ...
        const { data, error } = await supabaseAdmin.from("pl_machine_demand").insert({
            org_id: DEMO_ORG_ID,
...
async function DELETE(request) {
    try {
        ...
        const shiftType = shiftParamToDbValue(shift);
        const { error } = await supabaseAdmin.from("pl_machine_demand").delete().eq("org_id", DEMO_ORG_ID).eq("machine_code", machineCode).eq("plan_date", date).eq("shift_type", shiftType);
        if (error) throw error;
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            success: true
        });
...
//# sourceMappingURL=%5Broot-of-the-server%5D__7845d161._.js.map