...
CREATE TABLE IF NOT EXISTS public.hr_task_resolutions (
  ...
  note text,
  resolved_by uuid REFERENCES auth.users(id) ON DELETE SET NULL DEFAULT auth.uid(),
...
-- Fix nullability for existing tables (if table was created with NOT NULL)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public'
    AND table_name = 'hr_task_resolutions'
    AND column_name = 'resolved_by'
    AND is_nullable = 'NO'
  ) THEN
    ALTER TABLE public.hr_task_resolutions ALTER COLUMN resolved_by DROP NOT NULL;
  END IF;
END $$;
