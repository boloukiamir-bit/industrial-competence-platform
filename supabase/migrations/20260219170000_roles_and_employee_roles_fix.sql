-- Phase B: Role model lock-in FIX – forward-compatible schema upgrade.
-- This migration upgrades existing installations where public.roles already exists
-- but is missing newer columns (e.g., role_type).
...
-- Ensure uniqueness for (org_id, code)
DO $$
BEGIN
  -- If an existing UNIQUE constraint already exists, this will fail; we ignore duplicate_object.
  BEGIN
    CREATE UNIQUE INDEX idx_roles_org_code_unique ON public.roles(org_id, code);
...
BEGIN
  BEGIN
    CREATE POLICY roles_select_org
      ...
      FOR SELECT
      TO authenticated
      USING (org_id = (SELECT active_org_id FROM public.profiles WHERE id = auth.uid() LIMIT 1));
...
ALTER TABLE public.employee_roles
  ADD COLUMN IF NOT EXISTS site_id uuid NULL REFERENCES public.sites(id) ON DELETE CASCADE;
...
-- =============================================================================
-- 3) Seed: Spaljisten roles (idempotent)
-- =============================================================================
DO $$
DECLARE
  v_org_id uuid;
