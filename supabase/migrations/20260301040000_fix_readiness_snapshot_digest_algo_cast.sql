...
begin
  ...
  -- FIX: cast algo to text to avoid digest(bytea, unknown)
  return encode(digest(convert_to(canonical_text, 'UTF8'), 'sha256'::text), 'hex');
end;
$$;

comment on function public.compute_readiness_snapshot_payload_hash is
'Computes SHA-256 hex digest of canonical readiness snapshot JSON. Uses digest(convert_to(text,''UTF8''), ''sha256''::text) to avoid digest(bytea,unknown).';
