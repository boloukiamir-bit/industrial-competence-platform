-- Station-level breakdown for Industrial Readiness (drilldown API). Same logic as v1.1.
...
RETURNS TABLE (
  ...
  station_code text,
  station_name text,
  station_score numeric,
  station_reason_codes text[],
  required_operators_count int,
  eligible_operators_count int,
  compliance_blockers_count int,
  absence_ratio numeric,
  criticality_factor numeric
)
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  RETURN QUERY
  WITH
  shift_ctx AS (SELECT v_shift_date AS shift_date, v_shift_code AS shift_code),
  sa AS (
    ...
    WHERE sa.org_id = p_org_id AND sa.shift_id = p_shift_id
    GROUP BY sa.station_id
  ),
  roster_agg AS (
    SELECT r.station_id, COUNT(*)::int AS roster_headcount,
      COUNT(*) FILTER (WHERE r.status = 'GO')::int AS go_count
    FROM public.v_roster_station_shift_drilldown_pilot r
    WHERE r.org_id = p_org_id AND r.site_id = p_site_id
      AND r.plan_date::date = (SELECT shift_date FROM shift_ctx) AND r.shift_code = (SELECT shift_code FROM shift_ctx)
  ...
  illegal_per_station AS (
    ...
    LEFT JOIN public.employee_compliance ec ON ec.org_id = p_org_id AND ec.employee_id = sa.employee_id
  ...
  station_scores AS (
    SELECT si.station_id,
      CASE WHEN si.required_operators_count IS NULL OR si.required_operators_count = 0 THEN 0
      ...
      COALESCE((SELECT array_agg(reason ORDER BY reason) FROM (
        ...
        UNION ALL SELECT 'LOW_SKILL_COVERAGE' WHERE si.required_operators_count IS NOT NULL AND si.required_operators_count > 0
  ...
  breakdown AS (
    ...
    JOIN station_inputs si ON si.station_id = ss.station_id
  ...
  FROM breakdown b
  ORDER BY b.station_score ASC, b.station_code ASC;
END;
$$;
...
ALESCE(b.absence_ratio, 0)::numeric,
    b.criticality_factor
  FROM breakdown b
  ORDER BY b.station_score ASC, b.station_code ASC;
END;
$$;
