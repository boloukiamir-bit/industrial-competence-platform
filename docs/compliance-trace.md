...
| **Shared freeze logic** | `lib/server/readiness/freezeReadinessSnapshot.ts` | `createOrReuseReadinessSnapshot(params)`: fetches readiness-v3 + iri-v1, checks 1‑min duplicate window; on new insert calls RPC `insert_readiness_snapshot_chained` (org-scoped advisory lock, chain_position, previous_hash, canonical hash). Every insert persists v2 payload and tamper-evident `payload_hash`. Used by POST /api/cockpit/readiness-freeze and by the decision route. |

Snapshot content: readiness-v3 (legal/ops flags, overall status, reason_codes, samples) + IRI_V1 (score, grade) at decision time. **Self-contained v2 payload** (migration `20260228220000_readiness_snapshots_v2_payload.sql`): `overall_reason_codes`, `legal_blockers_sample`, `ops_no_go_stations_sample`, `engines`. **Integrity hash** (migration `20260228230000_readiness_snapshots_integrity_hash.sql`): `payload_hash`, `payload_hash_algo` (default `SHA256_V1`). **Ledger (append-only hash-chain)** (migrations `20260301010000_readiness_snapshots_hash_chain.sql`, `20260301020000_readiness_snapshots_chain_insert_rpc.sql`): table is immutable (no UPDATE/DELETE); new rows set `chain_position` = (max per org) + 1 and `previous_hash` = last snapshot’s `payload_hash`. Insert is concurrency-safe via `pg_advisory_xact_lock(hashtext(org_id::text))`. Canonical payload for hashing includes `previous_hash` and `chain_position` (between shift_code and legal_flag) so the hash commits the chain. Non–shift decisions do not create or link a snapshot.
...
- **Integrity verify:** `GET /api/readiness/snapshots/[id]/verify` — same auth; recomputes SHA-256 payload hash from canonical JSON (including `previous_hash`, `chain_position`) and compares to stored `payload_hash`. Returns `{ ok, snapshot_id, stored_hash, computed_hash, match, algo }`; if stored hash is null (pre-hash snapshots), `match: false`, `reason: "MISSING_HASH"`. Canonical payload: fixed field order, chain fields, `overall_reason_codes` sorted, `engines` keys sorted. See `lib/server/readiness/snapshotPayloadHash.ts`.
