...
| **Shared freeze logic** | `lib/server/readiness/freezeReadinessSnapshot.ts` | `createOrReuseReadinessSnapshot(params)`: fetches readiness-v3 + iri-v1, checks 1‑min duplicate window, inserts or returns existing snapshot. Every insert persists v2 payload: `overall_reason_codes`, `legal_blockers_sample`, `ops_no_go_stations_sample`, `engines`. Used by POST /api/cockpit/readiness-freeze and by the decision route. |

Snapshot content: readiness-v3 (legal/ops flags, overall status, reason_codes, samples) + IRI_V1 (score, grade) at decision time. **Self-contained v2 payload** (migration `20260228220000_readiness_snapshots_v2_payload.sql`): `overall_reason_codes` (text[]), `legal_blockers_sample` (jsonb), `ops_no_go_stations_sample` (jsonb), `engines` (jsonb, e.g. `{ "readiness":"V3", "iri":"IRI_V1", "compliance":"MATRIX_V2", "competence":"MATRIX_V2" }`). Only samples already produced by readiness-v3 are stored; no full by_employee/by_requirement arrays. Existing snapshots remain readable (defaults applied). Non–shift decisions do not create or link a snapshot.
...
- **Snapshot drilldown:** `GET /api/readiness/snapshots/[id]` — org-member auth; returns the snapshot row for the active org (404 if not found or wrong org). Response includes `snapshot.overall_reason_codes`, `snapshot.legal_blockers_sample`, `snapshot.ops_no_go_stations_sample`, `snapshot.engines`. `?debug=1` adds `_debug: { org_id, site_id }`.
- **Audit:** When viewing a focused governance event that is a cockpit decision, the audit event API enriches `meta.readiness_snapshot_id` from `execution_decisions`. The admin audit page shows a “Readiness Snapshot” section with the id, a “View snapshot” action, and when loaded: Engines line, reason_codes, and sample counts above the full JSON payload.
