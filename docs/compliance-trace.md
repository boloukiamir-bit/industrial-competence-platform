...
| **Shared freeze logic** | `lib/server/readiness/freezeReadinessSnapshot.ts` | `createOrReuseReadinessSnapshot(params)`: fetches readiness-v3 + iri-v1, checks 1‑min duplicate window, inserts or returns existing snapshot. Every insert persists v2 payload and tamper-evident `payload_hash` (SHA-256 of canonical JSON, algo `SHA256_V1`). Used by POST /api/cockpit/readiness-freeze and by the decision route. |

Snapshot content: readiness-v3 (legal/ops flags, overall status, reason_codes, samples) + IRI_V1 (score, grade) at decision time. **Self-contained v2 payload** (migration `20260228220000_readiness_snapshots_v2_payload.sql`): `overall_reason_codes`, `legal_blockers_sample`, `ops_no_go_stations_sample`, `engines`. **Integrity hash** (migration `20260228230000_readiness_snapshots_integrity_hash.sql`): `payload_hash` (text, SHA-256 hex of canonical JSON), `payload_hash_algo` (default `SHA256_V1`). New snapshots always get a hash; older rows remain null. Non–shift decisions do not create or link a snapshot.
...
- **Snapshot drilldown:** `GET /api/readiness/snapshots/[id]` — org-member auth; returns the snapshot row for the active org (404 if not found or wrong org). Response includes `snapshot.overall_reason_codes`, `snapshot.legal_blockers_sample`, `snapshot.ops_no_go_stations_sample`, `snapshot.engines`, and when present `snapshot.payload_hash`, `snapshot.payload_hash_algo`. `?debug=1` adds `_debug: { org_id, site_id }`.
- **Integrity verify:** `GET /api/readiness/snapshots/[id]/verify` — same auth; recomputes SHA-256 payload hash from canonical JSON and compares to stored `payload_hash`. Returns `{ ok, snapshot_id, stored_hash, computed_hash, match, algo }`; if stored hash is null (pre-hash snapshots), `match: false`, `reason: "MISSING_HASH"`. Canonical payload: fixed field order, `overall_reason_codes` sorted, `engines` keys sorted; hash = SHA-256(UTF-8(JSON)). See `lib/server/readiness/snapshotPayloadHash.ts`.
